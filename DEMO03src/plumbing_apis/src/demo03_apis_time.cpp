...
/* 
    ...
    注意：
        ...
        参数3：是否只执行一次
        参数4：是否自动启动（当设置为 false，需要手动调用 timer.star()）
...
int main(int argc, char *argv[])
{
    ...
    ros::NodeHandle nh;//注意：必须。 否则会导致时间API调用失败（再NodeHandle会初始化实践操作）
    
    // 2.获取当前时刻
    //now 函数会将当前时刻封装并返回
    //当前时刻：now 被调用执行的那一刻
    //参考系：1970年01月01日 00：00：00
    ros::Time rignt_now = ros::Time::now();
    ROS_INFO("当前时刻：%.2f",rignt_now.toSec());
    ROS_INFO("当前时刻：%.d",rignt_now.sec);
   ...
   */
   // ros::Timer timer = nh.createTimer(ros::Duration(1),cd);
   //ros::Timer timer = nh.createTimer(ros::Duration(1),cd,true);
   ros::Timer timer = nh.createTimer(ros::Duration(1),cd,false,false);
   timer.start();//手动启动
